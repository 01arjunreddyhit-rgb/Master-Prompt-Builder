...
const lockChoiceResults = async (election_id, conn) => {
  ...
  // Parameterized insert row by row (safe, idempotent via ON CONFLICT DO NOTHING)
  for (const t of tokens) {
    await db.execute(
      `INSERT INTO election_choice_results
       (election_id, student_id, token_number, course_id, original_status,
        is_auto_assigned, seat_id, round_confirmed, token_code)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT (election_id, student_id, token_number) DO NOTHING`,
      [election_id, t.student_id, t.token_number, t.course_id,
       t.status, t.is_auto_assigned ? true : false,
       t.seat_id || null, t.round_confirmed || null, t.token_code]
    );
  }
  ...
  try {
    ...
    const [summary] = await pool.execute(
      `SELECT
         ...
         SUM(CASE WHEN ecr.is_auto_assigned=TRUE THEN 1 ELSE 0 END) as auto_count,
         SUM(CASE WHEN ecr.is_auto_assigned=FALSE AND ecr.original_status IN ('CONFIRMED','BOOKED') THEN 1 ELSE 0 END) as self_count
       FROM courses c
...
const createSession = async (req, res) => {
  try {
    ...
    try {
      ...
      // Snapshot courses
      await conn.execute(
        `INSERT INTO allocation_session_courses (session_id, course_id, is_active, is_burst, confirmed_count)
         SELECT ?, course_id, is_active, is_burst, confirmed_count FROM courses WHERE election_id=?`,
        [session_id, election_id]
      );

      // Snapshot tokens
      await conn.execute(
        `INSERT INTO allocation_session_tokens (session_id, token_id, status, seat_id, round_confirmed, is_auto_assigned)
         SELECT ?, token_id, status, seat_id, round_confirmed, is_auto_assigned FROM student_tokens WHERE election_id=?`,
        [session_id, election_id]
      );
      ...
      res.json({
        success: true,
        message: 'Session created with full state snapshot.',
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO allocation_overrides (session_id, election_id, student_id, token_number, course_id, reason)
       VALUES (?,?,?,?,?,?)
       ON CONFLICT (session_id, student_id, token_number) DO UPDATE SET course_id=EXCLUDED.course_id, reason=EXCLUDED.reason`,
...
const restoreSession = async (req, res) => {
  ...
  try {
    ...
    // 1. Reset all seats for this election
    await conn.execute(
      'UPDATE seats SET is_available=TRUE, student_token_id=NULL WHERE election_id=?',
      [election_id]
    );

    // 2. Restore courses
    const [restoredCourses] = await conn.execute(
      'SELECT * FROM allocation_session_courses WHERE session_id=?',
      [session_id]
    );
    for (const rc of restoredCourses) {
      await conn.execute(
        'UPDATE courses SET is_active=?, is_burst=?, confirmed_count=? WHERE course_id=?',
        [rc.is_active, rc.is_burst, rc.confirmed_count, rc.course_id]
      );
    }

    // 3. Restore tokens
    const [restoredTokens] = await conn.execute(
      'SELECT * FROM allocation_session_tokens WHERE session_id=?',
      [session_id]
    );
    for (const rt of restoredTokens) {
      await conn.execute(
        'UPDATE student_tokens SET status=?, seat_id=?, round_confirmed=?, is_auto_assigned=? WHERE token_id=?',
        [rt.status, rt.seat_id, rt.round_confirmed, rt.is_auto_assigned, rt.token_id]
      );
      
      // 4. Update seat if token has one
      if (rt.seat_id) {
        await conn.execute(
          'UPDATE seats SET is_available=FALSE, student_token_id=? WHERE seat_id=?',
          [rt.token_id, rt.seat_id]
        );
      }
    }

    await conn.commit();
    res.json({ success: true, message: 'Session restored to live state.' });
  } catch (err) {
    await conn.rollback();
