...
const { expireCAV } = require('./cavController');
const { lockChoiceResults } = require('./resultController');
...
async function silentGenerateCAV(election_id) {
  try {
    ...
    await pool.execute(
      'INSERT IGNORE INTO election_cav (election_id, election_code, join_link) VALUES (?, ?, ?)',
  ...
  try {
    ...
    for (const student of students) {
      for (let t = 1; t <= courses.length; t++) {
        const token_code = `A${student.register_number}-T${t}`;
        await conn.execute(
