...
const { sendAllocationResult } = require('../config/email');
    ...
    try {
      await conn.execute(
        `INSERT INTO allocation_rounds (election_id, round_number, students_confirmed, capacity_settings)
         VALUES (?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE students_confirmed = students_confirmed + ?`,
        [election_id, round_number, confirmed, JSON.stringify({ course_id, capacity }), confirmed]
      );
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT s.student_id, s.name, s.register_number, s.section, s.email,
              ...
              GROUP_CONCAT(c.course_name SEPARATOR ', ') as pending_courses
       FROM student_tokens st
       JOIN students s ON st.student_id = s.student_id
       LEFT JOIN courses c ON st.course_id = c.course_id
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT c.course_id, c.course_name, c.subject_code, c.is_burst, c.is_active,
              ...
              SUM(CASE WHEN ecr.is_auto_assigned=0 AND ecr.original_status IN ('CONFIRMED','BOOKED') THEN 1 ELSE 0 END) as orig_self,
              SUM(CASE WHEN ecr.is_auto_assigned=1 THEN 1 ELSE 0 END) as orig_auto,
              ...
              SUM(CASE WHEN st.status IN ('BOOKED','CONFIRMED') AND st.is_auto_assigned=0 THEN 1 ELSE 0 END) as alloc_self,
              SUM(CASE WHEN st.status IN ('BOOKED','CONFIRMED') AND st.is_auto_assigned=1 THEN 1 ELSE 0 END) as alloc_auto
       FROM courses c
...
module.exports = { getRoundPool, confirmCourse, burstCourse, verifyAllocation, exportCSV, sendResultEmails, manualArrange, getUnallocated, getSteps, getAbacusSummary };
