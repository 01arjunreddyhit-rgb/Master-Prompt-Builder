...
/* ── Course card view ───────────────────────────────────────── */
function CourseCard({ c, i, onEdit, onDelete, onToggle }) {
...
export default function AdminCourses() {
  ...
  const openAdd  = () => { setEditCourse(null); setForm({ course_name: '', subject_code: '', description: '', min_enrollment: 45, max_enrollment: 75, classes_per_course: 1, credit_weight: 3.0 }); setShowModal(true); };
  const openEdit = c  => { setEditCourse(c); setForm({ course_name: c.course_name, subject_code: c.subject_code||'', description: c.description||'', min_enrollment: c.min_enrollment, max_enrollment: c.max_enrollment, classes_per_course: c.classes_per_course, credit_weight: c.credit_weight }); setShowModal(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else await api.post('/courses', { ...form, election_id: activeElection.election_id });
      ...
      <main className="main-content">
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px,1fr))', gap: 18 }}>
            {courses.map((c, i) => <CourseCard key={c.course_id} c={c} i={i} onEdit={openEdit} onDelete={handleDelete} onToggle={toggleActive} />)}
          </div>
        ) : (
                ...
                <tbody>
                  {courses.map((c, i) => (
                    <tr key={c.course_id}>
                      ...
                      <td><div style={{ display: 'flex', gap: 5 }}><button className="btn btn-ghost btn-sm" onClick={() => openEdit(c)}>Edit</button>{!c.is_burst && <button className={`btn btn-sm ${c.is_active?'btn-warning':'btn-success'}`} onClick={() => toggleActive(c)}>{c.is_active?'Pause':'On'}</button>}<button className="btn btn-danger btn-sm" onClick={() => handleDelete(c)}>✕</button></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {showModal && (
          <Modal title={editCourse ? 'Edit Course' : 'Add New Course'} onClose={() => setShowModal(false)}
            ...
            <div className="form-row">
              <div className="form-group"><label className="form-label">Min Enrollment</label><input className="form-input" type="number" value={form.min_enrollment} onChange={set('min_enrollment')} /></div>
              <div className="form-group"><label className="form-label">Max Enrollment</label><input className="form-input" type="number" value={form.max_enrollment} onChange={set('max_enrollment')} /></div>
            </div>
            {/* Live preview bar */}
